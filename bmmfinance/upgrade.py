...
with open('f:/code/bmmfinance/index.html', 'r', encoding='utf-8') as f:
    content = f.read()
...
new_css = """<style>
        ...
        .bmm-widget-root .final-cta {
            text-align: center; max-width: 800px; margin: 0 auto; padding: 60px; position: relative;
        }
...
# Add fixed BG right after widget root
bg_html = '''<div class="bmm-widget-root">
...
content = content.replace('<div class="bmm-widget-root">', bg_html, 1)

# Remove old hero-bg
content = re.sub(r'<div class="hero-bg">.*?</div>', '', content, flags=re.DOTALL)

# Update components with "glass" class
classes_to_update = {
...
for old, new in classes_to_update.items():
    content = content.replace(old, new)
    
# Fix image wrapper for padding glass effect
content = re.sub(
    r'(<img src="[^"]*"[^>]*>)',
    r'<div class="image-inner">\1</div>',
    content
)

# And fix the image overlay div to be inside the original container, currently it is right after img
content = content.replace('</div>\n                            <div class="image-overlay"></div>', '\n                            <div class="image-overlay"></div>\n                        </div>')

# Replace inline specific styles inside the document for better dark mode visibility
content = content.replace('background: rgba(255, 255, 255, 0.6);', '/* removed inline white */')
content = content.replace('background: rgba(255, 255, 255, 0.8);', '/* removed inline white */')
content = content.replace('background: rgba(255,255,255,0.8);', '/* removed inline white */')
...
for bg2 in ['style="background: var(--bg2);"', 'style="background: var(--bg2); padding-bottom: 120px;"']:
    content = content.replace(bg2, 'style="padding-bottom: 120px;"' if '120px' in bg2 else '')

# Avatar SVG Color updates
colors_map = {
    '#af8a4c': 'var(--c1)', 
    '#9fc5dc': 'var(--c3)', 
    '#e0c3a4': 'var(--c2)', 
    ...
    '#2A241F': '#ffffff' # Old dark color to white
}
for old, new in colors_map.items():
    content = content.replace(old, new)

with open('f:/code/bmmfinance/index.html', 'w', encoding='utf-8') as f:
    f.write(content)

print("HTML upgraded to glassmorphism")
