...
$path = "f:\code\bmmfinance\index.html"
...
$content = $content -replace '(?s)<div class="hero-bg">.*?</div>', ''

# Replace class attributes
$content = $content.Replace('class="hero-badge"', 'class="hero-badge glass"')
$content = $content.Replace('class="btn btn-outline"', 'class="btn btn-outline glass"')
$content = $content.Replace('class="btn-icon"', 'class="btn-icon glass"')
$content = $content.Replace('class="about-card hidden delay-3"', 'class="about-card glass hidden delay-3"')
$content = $content.Replace('class="interactive-image"', 'class="interactive-image glass"')
$content = $content.Replace('class="service-card hidden"', 'class="service-card glass hidden"')
$content = $content.Replace('class="feature-item hidden"', 'class="feature-item glass hidden"')
$content = $content.Replace('class="audience-pill hidden"', 'class="audience-pill glass hidden"')
$content = $content.Replace('class="final-cta"', 'class="final-cta glass"')

# Fix image inside Interactive Image context
$content = $content -replace '(<img src="[^"]*"[^>]*>)', '<div class="image-inner">$1</div>'
...
$content = $content.Replace('</div></div>
                            ...
                            <div class="image-overlay"></div>
                        </div>') # De-duplicate if needed

# Clean old inline background colors to fix contrast issues
$content = $content.Replace('background: rgba(255, 255, 255, 0.6);', '')
$content = $content.Replace('background: rgba(255, 255, 255, 0.8);', '')
$content = $content.Replace('background: rgba(255,255,255,0.8);', '')
$content = $content.Replace('background: linear-gradient(135deg, rgba(255, 255, 255, 1), rgba(255, 255, 255, 0.7));', '')
$content = $content.Replace('background: linear-gradient(180deg, var(--bg) 0%, var(--bg2) 100%);', '')
$content = $content.Replace('background: var(--bg2);', '')
$content = $content.Replace('style="background: var(--bg2); padding-bottom: 120px;"', 'style="padding-bottom: 120px;"')

# Adjust Avatar Colors
$content = $content.Replace('#af8a4c', 'var(--c1)')
$content = $content.Replace('#9fc5dc', 'var(--c3)')
$content = $content.Replace('#e0c3a4', 'var(--c2)')
$content = $content.Replace('#bce4ec', 'var(--c4)')
$content = $content.Replace('#f4ac94', 'var(--c5)')
$content = $content.Replace('rgba(175,132,76,0.2)', 'rgba(255,255,255,0.1)')
$content = $content.Replace('rgba(159,197,220,0.15)', 'rgba(255,255,255,0.1)')
$content = $content.Replace('#2A241F', '#ffffff')
